...
export default function PublicTrackingSearchPage(){
  ...
  async function submit(event:FormEvent){event.preventDefault();if(reference.trim().length<6)return;setLoading(true);setError("");setTracking(null);try{setTracking(await findPublicPackageTracking(reference));}catch{setError("Aucun colis public ne correspond à ce numéro de suivi. Vérifiez le numéro auprès de l’agence.");}finally{setLoading(false)}}
  return <main className="min-h-screen bg-white px-5 py-12 text-[#20272e] sm:py-20">
    <div className="mx-auto max-w-2xl">
      <header className="text-center"><div className="mx-auto grid h-12 w-12 place-items-center rounded-xl bg-[#eaf8f1] text-[#079456]"><PackageSearch size={24}/></div><p className="mt-5 text-[13px] font-semibold uppercase tracking-[.12em] text-[#079456]">SLAIVIO</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.03em]">Suivre un colis</h1><p className="mx-auto mt-3 max-w-lg text-[14px] leading-6 text-[#69747d]">Saisissez le numéro communiqué par votre agence pour consulter l’état et l’historique de votre colis.</p></header>
      ...
      {tracking&&<section className="mt-8 overflow-hidden rounded-xl border border-[#dfe4e7] bg-white">
        ...
        <div className="p-5"><h3 className="text-[14px] font-semibold">Historique</h3><div className="mt-4 space-y-4">{tracking.events.length?tracking.events.map((item,index)=><article key={`${item.occurred_at}-${index}`} className="border-l-2 border-[#b7dec9] pl-4"><p className="text-[13px] font-medium">{item.title}</p>{item.description&&<p className="mt-1 text-[12px] text-[#69747d]">{item.description}</p>}<time className="mt-1 block text-[11px] text-[#89929a]">{date(item.occurred_at)}</time></article>):<p className="text-[13px] text-[#69747d]">Le premier événement de suivi apparaîtra bientôt.</p>}</div></div>
      </section>}
    </div>
  </main>;
}
